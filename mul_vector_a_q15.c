...
void mul_vector_a_q15(int n, int32_t *dst, int16_t *src1, int16_t *src2)
{
    ...
    if (elapsed_time_fast > 0)
    {
        ...
        printf("The function mul_vector_a_q15 is completed. Elapsed time: %ld ms; time difference: %.2f%%.\n",
               elapsed_time_fast, diff_time);
    }
    else
    {
        printf("The function mul_vector_a_q15 is completed. Elapsed time: %ld ms; time difference: N/A.\n",
               elapsed_time_fast);
    }
    ...
    if (error_p)
    {
        fprintf(stderr, "ERROR: the function mul_vector_a_q15_fast did not compute correctly!\n");
        fprintf(stderr, "ERROR: i=%d, ref: dst[i]=0x%08x,, result: dst[i]=0x%08x\n", error_i, dst_ref[error_i], dst[error_i]);
    }
